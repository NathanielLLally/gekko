...
class PipelineFactory {
  static async create(settings) {
    return new Pipeline(await Promise.resolve(settings));
  }
  static async createInit(settings) {
    var p = new Pipeline(await Promise.resolve(settings));
    p.init();
    return p;
  }
  // Instantiate each enabled plugin
  static async loadPlugins(self) {
    // load all plugins
//    var self = this;
...
//    var self = this;
  ...
  static async createInit(settings) {
    var self = new Pipeline(await Promise.resolve(settings));
    self.init();
    async.series(
      [
