...
Trader.prototype.retry = function(method, warn, args, error) {
  ...
  if (error.code === 200) {
    console.log(`${this.name}: API rate limit exceeded! unable to call ${method}, will retry in 2 minutes`)
    wait = +moment.duration(120, 'seconds');
  }
  else {
    console.log(JSON.stringify(error));
    console.log(`${this.name}: ${warn}, will retry in 30 seconds`);
  }
...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  this.quadrigacx.api('transactions', reqData, _.bind(process, this));
};
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  var set = function(err, data) {
    ...
    if(!_.isNumber(assetAmount) || _.isNaN(assetAmount)) {
      console.log(`Quadriga did not return balance for ${this.asset.toLowerCase()}, assuming 0.`);
      assetAmount = 0;
    }

    if(!_.isNumber(currencyAmount) || _.isNaN(currencyAmount)) {
      console.log(`Quadriga did not return balance for ${this.currency.toLowerCase()}, assuming 0.`);
      currencyAmount = 0;
    }
  ...
  this.quadrigacx.api('balance', _.bind(set, this));
};
...
Trader.prototype.getTicker = function(callback) {
  ...
  this.quadrigacx.api('ticker', {book: this.pair}, _.bind(set, this));
};
...
Trader.prototype.addOrder = function(tradeType, amount, price, callback) {
  ...
  console.log(tradeType.toUpperCase(), amount, this.asset, '@', price, this.currency);
  ...
  var set = function(err, data) {
    ...
    var txid = data.id;
    console.log('added order with txid:', txid);

    callback(undefined, txid);
  };

  this.quadrigacx.api(tradeType, {
    book: this.pair,
    price: price,
    amount: amount
  }, _.bind(set, this));
...
Trader.prototype.getOrder = function(order, callback) {
  ...
  this.quadrigacx.api('lookup_oder', {id: order}, get);
}
...
Trader.prototype.checkOrder = function(order, callback) {
  ...
  this.quadrigacx.api('lookup_order', {id: order}, _.bind(check, this));
};
...
Trader.prototype.cancelOrder = function(order, callback) {
  ...
  this.quadrigacx.api('cancel_order', {id: order}, _.bind(cancel, this));
};
