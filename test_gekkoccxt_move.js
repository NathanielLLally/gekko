...
/*
  Object.keys(ccxt).map( (name) => { console.log(name) });
  process.exit(1);
  */
  [
    'fetchDepositAddress',
    ...
    ccxt.map( (e) => {
      if ( e.has[call] ) {
        e['getDepositAddress'] = e[call];
      }
      if (!e.has['withdraw']) {
        console.log("exchange "+e.name +" missing withdraw function!");
        process.exit(1);
      }
    });
  });

  ccxt[0].getDepositAddress('btc/usd');
  process.exit(1);
  return;
